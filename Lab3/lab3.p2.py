    ...
    def bfs(self, start):
        ...
        while queue:
            ...
            if node not in visited:
                ...
                queue.extend(neighbour for neighbour in self.graph.neighbors(node) if neighbour not in visited)
        return traversal
    ...
    def analyze(self):
        ...
        for size in sizes:
            for density in densities:
                ...
                vertices_visited = len(self.bfs(0))

                # Number of edges traversed
                edges_traversed = self.graph.number_of_edges()
        ...
        plt.title('BFS Execution Time for Different Graph Sizes')
        plt.legend()
        plt.grid(True)
        plt.show()
        ...
        plt.title('BFS Memory Usage for Different Graph Sizes')
        plt.legend()
        plt.grid(True)
        plt.show()
